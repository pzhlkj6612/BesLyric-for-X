...
int ConvertThread::buildOptionContent(OptionParseContext *octx)
{
    memset(octx, 0, sizeof(*octx));
    int nRet = init_parse_context(octx, groups, FF_ARRAY_ELEMS(groups));
    if(nRet != 0)
        return nRet;
...
int ConvertThread::init_parse_context(OptionParseContext *octx, const OptionGroupDef *groups, int nb_groups)
{
...
int ConvertThread::open_files(OptionGroupList *l,bool isInput)throw()
{
    int i, ret;
